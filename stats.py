...
SEQUENCE_MAX_POINTS = 11
STRUCTURE_MAX_POINTS = 4
CONVENTIONAL_MAX_POINTS = 6
...
def _prepare_df(path):
    ...
    df["SUM_sequence"] = pd.to_numeric(df["SUM_sequence"], errors="coerce")
    df["SUM_structure"] = pd.to_numeric(df["SUM_structure"], errors="coerce")
    df["SUM_conventional"] = pd.to_numeric(df["SUM_conventional"], errors="coerce")
...
def _score_percent(subdf, max_points, score_col):
    """Compute mean score percentage."""
    if len(subdf) == 0:
        return None

    values = subdf[score_col].dropna()

    if len(values) == 0:
        return None

    return (values.mean() / max_points) * 100.0
...
def _collect_scoring(groups, max_points, score_col):
    """Collect scoring statistics for all intron groups."""
    return {
        "alt_vs_const": (
            _score_percent(groups["alt"], max_points, score_col),
            _score_percent(groups["const"], max_points, score_col),
        ),
        "conv_vs_nonconv": (
            _score_percent(groups["conv"], max_points, score_col),
            _score_percent(groups["nonconv"], max_points, score_col),
        ),
        "within_conv": (
            _score_percent(groups["conv_alt"], max_points, score_col),
            _score_percent(groups["conv_const"], max_points, score_col),
        ),
        "within_nonconv": (
            _score_percent(groups["nonconv_alt"], max_points, score_col),
            _score_percent(groups["nonconv_const"], max_points, score_col),
        ),
    }
...
def _collect_composition(groups):
    ...
    n_alt = len(groups["alt"])
    n_const = len(groups["const"])
    ...
    return {
        "alt_vs_const": (
            _share_percent(n_alt, n_alt + n_const),
            _share_percent(n_const, n_alt + n_const),
        ),
        "conv_vs_nonconv": (
...
Alternative vs Constitutive
  Alternative:   {_fmt(sequence_scoring["alt_vs_const"][0])}
  Constitutive:  {_fmt(sequence_scoring["alt_vs_const"][1])}
...
Alternative vs Constitutive
  Alternative:   {_fmt(structure_scoring["alt_vs_const"][0])}
  Constitutive:  {_fmt(structure_scoring["alt_vs_const"][1])}
...
Alternative vs Constitutive
  Alternative:   {_fmt(conventional_scoring["alt_vs_const"][0])}
  Constitutive:  {_fmt(conventional_scoring["alt_vs_const"][1])}
...
Alternative vs Constitutive
  Alternative:   {_fmt(composition["alt_vs_const"][0])}
  Constitutive:  {_fmt(composition["alt_vs_const"][1])}
...
def _build_detailed_txt(groups):
    ...
    sections.append(
        _build_detailed_table(
            "DETAILED STATS: Alternative vs Constitutive",
            {
                "Alternative": groups["alt"],
                "Constitutive": groups["const"],
            },
        )
    )
...
def run_stats(inp_csv, out_txt, detailed=True):
    ...
    sequence_scoring = _collect_scoring(groups, SEQUENCE_MAX_POINTS, "SUM_sequence")
    structure_scoring = _collect_scoring(groups, STRUCTURE_MAX_POINTS, "SUM_structure")
    conventional_scoring = _collect_scoring(groups, CONVENTIONAL_MAX_POINTS, "SUM_conventional")
