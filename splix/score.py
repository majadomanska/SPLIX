...
def make_secondary_seq(intron_first20, intron_last20, window_len=20, spacer_len=20):
    """Build artificial sequence for RNAfold.
    By default, the sequence contains the first 20 nt and last 20 nt
    of the intron separated by a 20 nt N spacer. For rescue folding,
    a shorter window can be used.  
    """
    left_intron = to_rna(intron_first20)[:window_len]
    right_intron = to_rna(intron_last20)[-window_len:]
    spacer = "N" * spacer_len
...
def get_cross_pairs(structure, left_len=20, spacer_len=20, right_len=20):
    """
...
def fold_and_score(intron_first20, intron_last20, window_len=20, spacer_len=20):
    """Run RNAfold and calculate cross-pairing scores for one window size."""
    seq = make_secondary_seq(
        intron_first20,
        intron_last20,
    ...
    cross_pairs = get_cross_pairs(
        structure,
        left_len=window_len,
        spacer_len=spacer_len,
        right_len=window_len,
    )
...
def run_scoring(
    ...
    if max_pairs_score_max is None:
        max_pairs_score_max = observed_max_pairs_score

    if total_pairs_score_max is None:
        total_pairs_score_max = observed_total_pairs_score
    ...
    if max_pairs_score_max > observed_max_pairs_score:
        msg = (
            "WARNING: configured max_pairs_score_max "
            f"({max_pairs_score_max}) is higher than "
            f"the highest observed max_pairs_score "
            f"({observed_max_pairs_score}). "
            "This may compress normalized values "
            "and reduce score sensitivity."
        )
        print(msg)
        warnings.append(msg)

    if total_pairs_score_max > observed_total_pairs_score:
        msg = (
            "WARNING: configured total_pairs_score_max "
            f"({total_pairs_score_max}) is higher than "
            f"the highest observed total_pairs_score "
            f"({observed_total_pairs_score}). "
            "This may compress normalized values "
            "and reduce score sensitivity."
        )
        print(msg)
    ...
    secondary_df["max_pairs_score_normalized"] = secondary_df[
        "max_pairs_score"
    ].apply(lambda x: normalize_score(x, max_pairs_score_max))

    secondary_df["total_pairs_score_normalized"] = secondary_df[
        "total_pairs_score"
    ].apply(lambda x: normalize_score(x, total_pairs_score_max))
    ...
    out["SUM_sequence_normalized"] = (out["SUM_sequence"] / 11).round(2)
    ...
    out["SUM_structure_normalized"] = (out["SUM_structure"] / 4).round(2)
