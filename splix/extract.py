...
def build_intron_candidate(intron, genome):
    ...
    if not c_or_n and len(intron_seq) >= 40:
...
def run_extract(gff_path, fasta_path, out_csv):
    ...
    fasta_scaffolds = set(genome.keys())
    gff_scaffolds = {r.scaffold for r in records}
    missing_scaffolds = sorted(gff_scaffolds - fasta_scaffolds)

    for scaffold in missing_scaffolds:
        warnings.append(
            f"WARNING: scaffold {scaffold!r} is present in GFF but not in FASTA. "
            "Records on this scaffold will be skipped."
        )
    ...
    for intron in introns:
        try:
            ...
            msg = (
                f"WARNING: skipping intron "
                f"{intron.rec_id or intron.name or intron.feature} "
                f"on scaffold {intron.scaffold!r}: {e}"
            )
            print(msg)
            warnings.append(msg)
            continue
    ...
    for cand in deduped:
        for warning in cand.warnings:
            warnings.append(
                f"WARNING: intron {cand.stable_id}: {warning}"
            )
    ...
    return {
        ...
        "out_csv": str(out_csv),
        "warnings": warnings,
        "missing_scaffolds_in_fasta": missing_scaffolds,
    }
